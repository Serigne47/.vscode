...
"""
        ...
        try:
            # Appeler le LLM avec le contexte COMPLET
    ...
    def _build_evidence_chunks(
        ...
        for chunk, details in zip(original_chunks, chunks_details):
            ...
            if 'metadata' in chunk:
                # Ajouter char_count si disponible
                if 'char_count' in chunk['metadata']:
                    evidence_chunk['metadata']['char_count'] = chunk['metadata']['char_count']
                
                # Ajouter breadcrumb si disponible
                if 'breadcrumb' in chunk['metadata']:
                    evidence_chunk['metadata']['breadcrumb'] = chunk['metadata']['breadcrumb']
                
                # Ajouter chunk_id si disponible
                if 'chunk_id' in chunk['metadata']:
                    evidence_chunk['metadata']['chunk_id'] = chunk['metadata']['chunk_id']
        ...
        for source in retrieval_result.sources[:3]:  # Max 3 sources
            ...
            if source_key not in seen_sources:
                sources.append({
                    ...
                    'snippet': source.get('text', '')[:100] if 'text' in source else ''
                })
